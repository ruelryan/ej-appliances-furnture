...
| `v_collector_day` | 0012 | Per-collector per-day roll-up of collection entries (counts by disposition, cash/online/posted/pending totals) — the daily report basis. |
...
| `post_collection_entry(entry, receipt_no, receipt_type)` | owner/admin | Turns a pending `collected` entry into a real payment via `record_payment`, links `payment_id`. **Same act as recording a payment on the Contracts tab** — doing both creates two payments for one collection. |
| `link_collection_payment(entry, payment)` | owner/admin | 0030. Closes a pending `collected` entry against a payment that ALREADY exists (the Contracts-tab path), setting `status='posted'` + `payment_id` and **creating nothing**. Validates same contract, payment not voided, payment not already linked to another entry. Atomic conditional update — do not refactor to select-then-update. |
| `cancel_collection_entry(entry, reason?)` | owner/admin any; collector own | Pending entries only. |
| `record_remittance(collector, amount, remitted_on?, note?)` | owner/admin | 0030. `RMT####`. Guards: target must be `role='collector'` (not `is_collector()` — a deactivated collector must still be able to hand in their last cash), amount > 0, date not in the future. Deliberately NOT capped against `cash_on_hand`. |

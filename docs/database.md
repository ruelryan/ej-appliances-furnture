...
| `close_contract(id)` | owner | Sets `payment_status = 'closed'`. Since 0032 it raises if the contract does not exist or is already closed, instead of reporting success and doing nothing. Closing with a balance outstanding stays allowed — a write-off, a settlement or a repossession all end a contract with money on the books. There is no reopen RPC: the owner sets the status back on the contract Edit page. |

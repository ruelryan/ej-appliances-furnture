...
Everything of consequence in this app lives in Postgres (Supabase project `trjlqcvhrgggcvsxxaml`), defined by migrations `supabase/migrations/0001` through `0032`. The schema follows three rules with almost no exceptions: money and time math live in SQL (functions and views), all writes go through `SECURITY DEFINER` RPC functions that generate race-safe IDs from `id_counters`, and Row Level Security scopes what each role can read. This document catalogs the 29 tables by module, the views (including the frozen-view rules that have bitten this project twice), the full RPC catalog with role guards, and the triggers. For how the app calls into all this, see [architecture.md](architecture.md); for the role model, see [roles-and-permissions.md](roles-and-permissions.md); for why some of these rules exist legally, see [business-rules-legal.md](business-rules-legal.md).
...
- **`audit_log`** — one row per changed field, written by the `audit_row_changes()` trigger on updates to contracts, payments, customers, time_records, employee_rates, time_correction_requests, payslips, and (0031) collection_entries, remittances, cash_advances and cash_advance_expenses. Owner-only read; nobody writes directly. Note it is `after update` only — inserts are not audited.
...
- **`remittances`** (0030) — one row per hand-over of field cash to the office, `RMT####`. `collector_id`, `amount > 0`, `remitted_on`, `received_by` (nullable — only so a future repair migration can write without a JWT), `note`, and void-style `cancelled_by`/`cancelled_at`/`cancel_reason`. **Never deleted**; a wrong amount is cancelled and re-recorded. Deliberately NOT linked to individual `collection_entries`: the cadence is flexible, so one remittance may settle parts of several days. Collector reads own rows only; writes are RPC-only.
...
| `v_collector_day` | 0012 | Per-collector per-day roll-up of collection entries (counts by disposition, cash/online/posted/pending totals) — the daily report basis. 0031 appended `cancelled_cash` and `cancelled_count`, because a cancelled entry silently leaves `cash_on_hand`. |
| `v_collector_month` | 0030 | The same roll-up bucketed by month — the history behind the remittance ledger. Entries only, no remittance column: under a flexible cadence a remittance does not belong to the month of the entries it settles. Also gained `cancelled_cash`/`cancelled_count` in 0031. |
| `v_collector_remittance` | 0030 | Per collector: `cash_collected`, `online_collected`, `total_collected`, `remitted_total`, **`cash_on_hand`** (= cash collected − remitted), `last_entry_on`, `last_remitted_on`. Counts pending AND posted entries — posting is bookkeeping, not a hand-over — and online contributes zero by construction. Ends with `and (can_post_payments() or p.id = auth.uid())`, which is load-bearing: `profiles_select` is a bare `is_active_user()`, so without it a collector would see a row per colleague reading as genuine zeroes. 0031 widened the row filter from `role = 'collector'` to also include anyone with entries or remittances on the books — otherwise promoting a collector to admin deleted their outstanding cash liability from the ledger, and `record_remittance` would refuse the hand-over. |
| `v_collector_month` | 0030 | Per collector per month: `entries`, `collected_count`, `promised_count`, `cash_total`, `online_total`, `posted_total`. Entries-only on purpose — under a flexible remittance cadence, bucketing hand-overs by month would manufacture a shortfall in one month and a surplus in the next. |
| `v_entry_payment_candidates` | 0030 | Pending collected entries paired with a payment that already exists for them (same contract, exact amount, within 7 days, not voided, not already linked). Strict on purpose: a false positive would invite closing an entry against somebody else's money. |
...
| `v_cashflow_daily` | 0033 | Unvoided payments summed per calendar day (`day`, `payment_count`, `collected`). Drives the 30-day bars on the owner dashboard. Purely additive — depends only on `payments`, so it is NOT a member of the `v_contract_financials` drop-cascade. **A day with no payments has NO ROW**, so any caller drawing a fixed window must fill the gaps or the chart misreports how often money arrives. |
...
| `record_payment(contract, date, amount, receipt_no, receipt_type, reference_no?)` | owner/admin | Inserts a `PAY####` payment. The only path into `payments`. Since 0032 it refuses a **closed** contract and any amount more than **₱100 above the outstanding balance** — the allowance covers rounding and goodwill on a final payment while still catching a misplaced digit, which used to drive `remaining_balance` negative and make the views read the account as fully paid. The balance is computed from `contracts.total_price` minus unvoided payments, not from `v_contract_financials` (that view is `security_invoker` and would re-enter RLS from inside a definer function). |
| `void_payment(id, reason)` / `unvoid_payment(id)` | owner | Void / restore — payments are never deleted. Voiding a payment that came from a collection entry leaves that entry `posted`; use `unlink_collection_payment` to put it back in the queue. |
| `book_sale(contract, invoice_no, sales_date, note?)` | owner + admin | Enters a contract in the sales book (0041). Snapshots `cash_price` (not `total_price`), derives the branch from `item_type` and the VAT split from `bir_split()` — none of the three is accepted from the caller. Reads base tables, never `v_contract_financials`, which is `security_invoker` and would re-enter RLS from inside a definer function. Translates the two unique-index violations into messages a person can act on. |
| `cancel_sale_entry(id, reason)` | owner + admin | Cancels, never deletes. Frees the invoice number and returns the sale to the not-yet-booked queue. |
| `record_bir_expense(...)` | owner + admin | Inserts a purchase-journal row (0039). Computes the VAT split via `bir_split()` and derives `period_key` from the date — neither is accepted from the caller. |
| `update_bir_expense(...)` | owner + admin | Same derivations. The `voided_at is null` predicate is in the WHERE and raises on `not found`, never a read-then-write. |
| `void_bir_expense(id, reason)` | owner + admin | Voids, never deletes. A reason is required. `bir_expenses` has **no delete policy at all**. |
| `upsert_bir_supplier(...)` | owner + admin | Creates or edits a supplier's BIR identity (`tin`, `vat_registered`, `bir_name`). |
| `bir_split(gross)` | (immutable helper) | VAT-inclusive gross → (vatable, input tax). Rounds the base and takes the tax as the remainder so the columns foot. Mirrored in TS by `birSplit()` with golden cases from the real sheet. |
| `can_see_bir()` / `can_manage_bir()` / `is_bookkeeper()` | – | Role helpers added by 0039. **`is_active_user()` was redefined by 0039 to exclude `bookkeeper`** — see [modules/bir.md](modules/bir.md), it is the single most surprising thing in the schema. |
| `close_contract(id)` | owner + admin | Sets `payment_status = 'closed'`. Since 0032 it raises if the contract does not exist or is already closed, instead of reporting success and doing nothing. **Widened from owner-only to `can_post_payments()` in 0038** — the admin records the payments, so the admin is who sees an account reach zero. Closing with a balance outstanding stays allowed — a write-off, a settlement or a repossession all end a contract with money on the books; the UI states the exact balance before it will proceed. |
| `reopen_contract(id)` | owner | **New in 0038.** Sets `payment_status` back to `'open'` so payments can be recorded again; raises if the contract is missing or already open. Deliberately narrower than closing — closing is routine bookkeeping, reversing someone else's close is not. Before 0038 closing was a one-way door with no path back except a hand-written UPDATE. |
...
| `post_collection_entry(entry, receipt_no, receipt_type, force?)` | owner/admin | Turns a pending `collected` entry into a real payment via `record_payment`, links `payment_id`. **Same act as recording a payment on the Contracts tab** — doing both creates two payments for one collection, which is why 0031 made it **refuse** when `v_entry_payment_candidates` already matches this entry unless `force` is passed. Locks the entry `for update` and the final write carries `and status = 'pending'`: that pairing is the fix for the double-post race the 0029 header named. |
| `link_collection_payment(entry, payment)` | owner/admin | 0030. Closes a pending `collected` entry against a payment that ALREADY exists (the Contracts-tab path), setting `status='posted'` + `payment_id` and **creating nothing**. Validates same contract, payment not voided, payment not already linked, and — since 0031 — **exact amount and a date within ±7 days**, mirroring `v_entry_payment_candidates`. Without the amount check a ₱10,000 entry could be closed against a ₱500 payment. The one-payment-one-entry rule is now held by a unique index, not by the pre-check. |
| `unlink_collection_payment(entry, reason?)` | owner/admin; **owner only** if the payment is still live | 0031. Returns a posted entry to `pending` and clears `payment_id`. The remedy for a mis-link, and for the entry stranded when the owner voids a payment that came from a collection — before this it left every worklist with no way back. |
| `cancel_collection_entry(entry, reason?)` | owner/admin any; collector own | Pending entries only, atomic conditional update since 0031. A cancelled entry leaves `cash_collected`, so this is the one act that moves a collector's `cash_on_hand` without a hand-over: it is audited (0031) and surfaced as `cancelled_cash` on the day and month views. |
| `record_remittance(collector, amount, remitted_on?, note?)` | owner/admin | 0030. `RMT####`. Guards: target must be `role='collector'` (not `is_collector()` — a deactivated collector must still be able to hand in their last cash) **or, since 0031, anyone with uncancelled collections on the books**, so a role change cannot strand field cash. Amount > 0, date not in the future. Deliberately NOT capped against `cash_on_hand`. |
| `cancel_remittance(id, reason?)` | **owner only** | 0030. Void-style; the amount returns to the collector's `cash_on_hand`. Atomic conditional update, so two concurrent cancels cannot both succeed. |
...
| `audit_contracts` / `audit_payments` / `audit_customers` (+ DTR/payroll siblings, and the cash tables since 0031: `collection_entries`, `remittances`, `cash_advances`, `cash_advance_expenses`) | after update | `audit_row_changes()` | Diffs old vs new jsonb and writes one `audit_log` row per changed field (skips `created_at`/`updated_at`). Cancelling an entry or a remittance IS an update, so the `after update` trigger catches it unmodified. |

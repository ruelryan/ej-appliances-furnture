...
- **Unlink** (`unlink_collection_payment`, 0031): returns a posted entry to `pending` and clears `payment_id`. Admin may unlink once the payment is voided; unlinking a live payment is owner-only, because it asserts that the payment and the collection are not the same money. This is the remedy for a mis-link, and for the entry left stranded when a payment that came from a collection is voided — before 0031 such an entry dropped out of every worklist with no way back.
...
**Cancelling is the one act that moves a collector's balance without a hand-over**, and it is deliberately still available to the collector — a mis-keyed amount in the field should be fixable by the person who made it. The control is visibility, not permission: since 0031 every update to `collection_entries`, `remittances`, `cash_advances` and `cash_advance_expenses` writes to `audit_log`, and `v_collector_day`/`v_collector_month` carry `cancelled_cash`, shown as its own tile and column on `/collections/report` and as a column on `/collections/remittances`. A cancelled collected entry should always be checked against the collector's receipt booklet — the paper is the independent record.
...
- **A voided payment does not refund custody.** The collector still took the cash; voiding is a ledger correction. If the void is because the cash never existed, cancel the *entry* — that is what removes it from the balance. Cancel only takes a `pending` entry, so for one already posted the order is `unlink_collection_payment` first, then cancel. Before 0031 there was no unlink, which made this documented remedy impossible after posting and left a fictitious remittance as the only way to zero a bogus balance.
...
`v_entry_payment_candidates` finds those pairs (same contract, exact amount, within seven days, payment not voided and not already linked) and the board offers **`link_collection_payment`**, which closes the entry against the payment that already exists and creates nothing. One payment can back only one entry — enforced since 0031 by a unique index on `collection_entries.payment_id`, not by the in-function pre-check, which was an unlocked read that two concurrent links could both pass. The same migration made the RPC re-check the match itself (exact amount, ±7 days): the view is only a suggestion engine, and the function used to accept any pair on one contract, so a ₱10,000 entry could be closed against a ₱500 payment and the difference would vanish from the books.

Posting is still possible on a matched entry — a genuine second payment of the same size can happen — but the dialog warns in red, relabels the button "Post a second payment", and now passes an explicit `force` flag: **`post_collection_entry` refuses a matched entry without it.** The warning used to be React state only, which is advisory, because the browser holds the anon key and can call the RPC directly.
